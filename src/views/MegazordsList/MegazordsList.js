...
function CreateMegazord(props) {
  const { onClose, value: valueProp, open, user, api_functions, ...other } = props;
  ...
  const addToList = (e) => {
    e.preventDefault();
    resAccounts.push({...bitcloutAccount})
    setAccounts(resAccounts);
  }

  const removeFromList = (e, id) => {
    e.preventDefault();
    resAccounts = resAccounts.filter(it => it.id !== id);
    setAccounts(resAccounts);
  }
      ...
      <DialogContent dividers>
        <div className={classes.addOwnerWrapper}>
          <Input
            ...
            value={inputAccount}
            inputProps={{
              placeholder: "Add BitClout account",
              inputProps: {
  ...
  return (
    <div>
      <CreateMegazord
        ...
        open={open}
        user={user}
        api_functions={props.api_functions}
        onClose={handleClose}
        value={value} />
