...
function SeedPhraseDialog(props) {
  const { onConfirm, onClose, value: valueProp, open, user, api_functions, ...other } = props;
  ...
  return (
    <Dialog
      ...
      {...other}
    >
        <DialogTitle id="dialog-title"> Store your seed phrase</DialogTitle>
        <DialogContent dividers>

          <Warning variant="body1" align="left">
          </Warning>
            <MuiTypography style={{color:dangerColor[0]}}>
  ...
  const inputOnChange = (event) => {
    ...
    sleepVar = setTimeout(() => {
      api_functions.getBitcloutAcc('', inputAccount).then(data => {
        ...
        if ([...accounts].some(it => it.id === data.id)) {
...
export default function MegazordsList(props) {
  ...
  var megazords = user.megazords ? Object.values(user.megazords) : [];
  megazords = megazords.map((item, i) => {
    item[i].id = Object.keys(user.megazords)[i];
    return item;
  })
  ...
  const handleConfirm = () => {
    ...
    api_functions.createMegazord(_zords).then(() =>  {
